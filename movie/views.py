...
class EditReviewView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        movie.average_rating = Review.objects.filter(movie=movie).aggregate(
            avg_rating=Avg("rating")
        )["avg_rating"]
        movie.save()
...
class MyProfileView(LoginRequiredMixin, DetailView):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            user = form.save(
                commit=True
            )  # Enregistre l'utilisateur avec les nouvelles informations
            ...
            if password:  # Vérifie si un mot de passe a été soumis
                ...
                messages.success(
                    request, "Profil modifié avec succès"
                )  # Message de succès sans changement de mot de passe
